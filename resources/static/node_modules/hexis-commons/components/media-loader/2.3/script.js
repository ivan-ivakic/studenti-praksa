App.mediaLoader = {
  ...
  parseNode: function(node, callback) {
    ...
    var pNode = {};
      ...
      pNode.root = node.getAttribute('data-root') || null; // You can overwrite the default root path
      ...
      pNode.dataSrc = App.mediaLoader.parseSrc(node.getAttribute('data-src'), pNode.root);
  ...
  ------------------------------------------------------------------------------------*/
  parseSrc: function(dataSrc, root) {
    ...
    sets.forEach(function(node, i){
      ...
      // Full URL ?
      if (params[0].indexOf('http') === 0) {
        finalPath = params[0];
      }

      // Alternative root path?
      else if (root !== null) {
        finalPath = root + params[0];
      }

      // Default root path
      else {
        finalPath = App.mediaPath + params[0];
      }
