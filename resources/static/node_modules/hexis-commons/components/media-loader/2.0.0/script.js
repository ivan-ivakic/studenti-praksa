App.mediaLoader = {
  ...
  check: function(target) {
    ...
    array(localNodes).forEach(function(node) {
      ...
      if (App.checkIfVisible(node, scope)=== true) {
  ...
  load: function(node, callback) {
    ...
    if (pNode.isNative) {
      ...
      // Streaming media may load just partially, which should also return a success
      if (pNode.isStreamingMedia) {
        pNode.el.oncanplay = function(){ pNode.status='done'; App.mediaLoader.onComplete(pNode, callback); };
      }

      // Android video needs controls?
      // if (pNode.isStreamingMedia) {
      //   pNode.el.oncanplay = function(){ pNode.status='done'; App.mediaLoader.onComplete(pNode, callback); };
      // }
  ...
  parseNode: function(node, callback) {
    ...
    var pNode = {};
      ...
      pNode.src = null; // This will be determined later
    ...
    // Otherwise, determine which of the multiple sources matches
      ...
      for(var i=pNode.dataSrc.length-1; i>=0; i--){
        ...
        if (pNode.dataSrc[i].conditions.length < 1 && pNode.src === null) {
          pNode.src = pNode.dataSrc[i].src;
        }

        // Otherwise, check if all conditions match
        else if (App.mediaLoader.checkConditions(pNode.dataSrc[i].conditions) === true  && pNode.src === null) {
          pNode.src = pNode.dataSrc[i].src;
        }
      };
    }
  ...
  waitForAll: function(target, callback, delayPriority) {
    ...
    function checkIfAll() {
      if(t.debug && t.verbose){ console.log('App.mediaLoader.checkIfAll()', nodesLoaded, localNodes.length); }
