...
App.checkMediaQuery = function (query, explicit) {
  var match = false;
  var reverse = query.charAt(0) === '!';

  // Default to coarse
  var explicit = explicit || false;

  // If the condition has a "@" character, then we're matching resolutions...
  if (query.indexOf('@') > -1) {
    for (var i=0; i < ux.device.resolution.length; i++) {
      if (!reverse && ux.device.resolution.indexOf(query) > -1) { match = true; }
      else if (reverse && ux.device.resolution.indexOf(query) < 0) { match = true; }
    }
  }

  // iOS...
  else if (query.indexOf('ios') > -1) {
    if (!reverse && App.device.os === 'ios') { match = true; }
    else if (reverse && App.device.os !== 'ios') { match = true; }
  }

  // Android...
  else if (query.indexOf('android') > -1) {
    if (!reverse && App.device.os === 'android') { match = true; }
    else if (reverse && App.device.os !== 'android') { match = true; }
  }
  ...
  // Active media queries...
    ...
    if( explicit === true && ux.device.screenSize.all[ux.device.screenSize.all.length-1] === query ){ match = true; }
      ...
      for (var i = 0; i < ux.device.screenSize.all.length; i++) {
        if (!reverse && ux.device.screenSize.all.indexOf(query) > -1) { match = true; }
        else if (reverse && ux.device.screenSize.all.indexOf(query) < 0) { match = true; }
      }
    }
  }

  // If matches are not found, return false
  return match;
}
...
------------------------------------------------------------------------------------*/
App.checkIfVisible = function(el, scopeSelector, tresholdMultiplier) {
  ...
  if (scope === document.body) { scopeHeight = ux.viewport.height; }
  else { scopeHeight = scope.getBoundingClientRect().height; }
  ...
  if (
    ...
    // console.log(false, bounds.top, bounds.height);
    return false;
  }
}
